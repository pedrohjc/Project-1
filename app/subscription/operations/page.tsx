...
const sectors = [
  {
    title: 'Financeiro',
    agents: [
      { name: 'Conciliação de recibos', desc: 'Notas de entrada → classificação + alertas de inconsistência.' },
      { name: 'Cobrança inteligente', desc: 'Overdue → mensagens padronizadas + follow-up automático.' },
      { name: 'Fluxo de caixa', desc: 'Contas + previsão → resumo semanal consolidado.' },
    ],
  },
  {
    title: 'Marketing',
    agents: [
      { name: 'Calendário editorial', desc: 'Briefing → pauta organizada + agenda de publicação.' },
      { name: 'Análise de campanhas', desc: 'Métricas → insights acionáveis + próximos passos.' },
    ],
  },
  {
    title: 'Produção',
    agents: [
      { name: 'Checklist de linha', desc: 'Ordem de produção → checklist + pendências.' },
      { name: 'Controle de qualidade', desc: 'Registros → alertas + relatório de conformidade.' },
      { name: 'Planejamento de insumos', desc: 'Consumo histórico → previsão + pedido sugerido.' },
    ],
  },
  {
    title: 'Atendimento',
    agents: [
      { name: 'Triagem de chamados', desc: 'Entrada → categoria + prioridade + responsável.' },
      { name: 'Follow-up automático', desc: 'Ticket aberto → lembrete + status atualizado.' },
    ],
  },
]
...
const included = [
  ...
  'Chat de suporte',
  '1 call de acompanhamento por mês',
  'Atualizações constantes',
  'Cancelamento a qualquer momento',
]

const WHATSAPP = 'https://wa.me/5500000000000?text=Quero%20agendar%20minha%20call%20mensal%20-%20Plano%20Operations'
    ...
    <div style={{ minHeight: '100vh', background: 'var(--balance-bg)' }}>
      ...
      <div className="container" style={{ padding: '60px 20px', maxWidth: '1000px' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <div style={{ display: 'inline-block', background: 'var(--balance-flow)', color: 'white', padding: '5px 14px', borderRadius: '999px', fontSize: '0.72rem', fontWeight: '600', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '14px' }}>
            Mais popular
          </div>
        ...
        {sectors.map((sector) => (
          <div key={sector.title} style={{ marginBottom: '2.5rem' }}>
            <h2 style={{ fontSize: '1.2rem', fontWeight: '700', marginBottom: '12px', color: 'var(--balance-flow)' }}>{sector.title}</h2>
            <div style={{ display: 'grid', gap: '12px' }}>
              {sector.agents.map((agent) => (
                <div key={agent.name} className="card" style={{ padding: '20px' }}>
                  ...
                  <p style={{ color: 'var(--balance-text-light)', fontSize: '0.95rem', lineHeight: '1.6' }}>{agent.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="card" style={{ padding: '24px', marginBottom: '3rem', borderTop: '3px solid var(--balance-flow)' }}>
          <h3 style={{ fontSize: '1.1rem', marginBottom: '10px' }}>Call mensal de acompanhamento</h3>
          <p style={{ color: 'var(--balance-text-light)', fontSize: '0.95rem', lineHeight: '1.6', marginBottom: '16px' }}>
            Você tem direito a 1 call por mês com a equipe Balance para revisar resultados, ajustar fluxos e tirar dúvidas. Agende pelo WhatsApp:
          </p>
